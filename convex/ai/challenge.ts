...
async function doGenerateChallenge(
  ...
  // Authorization: conceptId is caller-supplied, so a challenge may only be
  // generated by the concept's owner. Throw the same "not found" either way so
  // we don't reveal whether someone else's concept exists.
  if (concept.userId !== requireUserId) throw new Error("Concept not found");
...
export const generateChallenge = action({
  ...
  handler: async (ctx, args): Promise<ChallengeResult> => {
    ...
    return doGenerateChallenge(ctx, args.conceptId, level, userId);
  },
});
...
export const preGenerateForEntry = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    for (const concept of concepts) {
      try {
        await doGenerateChallenge(ctx, concept._id, challengeLevel, args.userId);
      } catch (error) {
        console.error(
...
export const regenerateChallenge = action({
  ...
  handler: async (ctx, args): Promise<ChallengeResult> => {
    ...
    if (!userId) throw new Error("Not authenticated");
    const level = args.challengeLevel ?? "intermediate";
    // Verify ownership BEFORE invalidating, so a caller can't wipe another
    // user's cached challenges by passing their conceptId.
    const concept = await ctx.runQuery(internal.challengeHelpers.getConcept, {
